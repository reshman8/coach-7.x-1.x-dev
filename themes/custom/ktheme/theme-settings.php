 ...
 */
function ktheme_form_system_theme_settings_alter(&$form, &$form_state, $form_id = NULL) {
  if (isset($form_id)) {
    return;
  }
  ...
  // Breadcrumbs
  $form['ktheme']['breadcrumb'] = array(
    '#type' => 'fieldset',
    '#weight' => '5',
    '#title' => t('Breadcrumbs'),
  );
  $form['ktheme']['breadcrumb']['bd']['breadcrumb_display'] = array(
    '#type' => 'select',
  ...
  $form['background']['settings']['path_background'] = array(
    ...
    '#default_value' => variable_get('ktheme:path_background', ''),
  );
  ...
  $form['background']['settings']['background_color_value'] = array(
    ...
    '#default_value' => variable_get('ktheme:background_color_value', ''),
    '#field_prefix' => '#',
    '#states' => array(
      'invisible' => array(
  ...
  $form['#validate'][] = 'ktheme_theme_settings_validate';
  $form['#submit'][] = 'ktheme_theme_settings_submit';
}

function ktheme_theme_settings_validate($form, &$form_state) {
// Handle file uploads.
...
function ktheme_theme_settings_submit($form, &$form_state) {
  ...
  if (!empty($values['path_background'])) {
    ...
    variable_set('ktheme:path_background', $values['path_background']);
  }
  
  if (!empty($values['background_color_value'])) {
    variable_set('ktheme:background_color_value', $values['background_color_value']);
  }
  
  //Delete background settings when the default background is used
  if ($values['default_background'] == 1) {
  	variable_del('ktheme:path_background');
    variable_del('ktheme:background_color_value');
  }
}
